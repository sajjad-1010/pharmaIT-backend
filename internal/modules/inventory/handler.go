...
func (h *Handler) addMovement(c *fiber.Ctx) error {
	...
	if req.MedicineID == uuid.Nil {
		return response.Fail(c, appErr.BadRequest("INVALID_MEDICINE_ID", "medicine_id is required", nil))
	}
	switch req.Type {
...
// @Security BearerAuth
// @Produce json
// @Param medicine_id query string true "medicine id"
...
func (h *Handler) getStock(c *fiber.Ctx) error {
	...
	raw := strings.TrimSpace(c.Query("medicine_id"))
	if raw == "" {
		return response.Fail(c, appErr.BadRequest("INVALID_MEDICINE_ID", "medicine_id is required", nil))
	}
	medicineID, err := uuid.Parse(raw)
	if err != nil {
		return response.Fail(c, appErr.BadRequest("INVALID_MEDICINE_ID", "medicine_id is invalid", nil))
	}

	available, err := h.svc.CurrentAvailable(c.UserContext(), nil, wholesalerID, medicineID)
	if err != nil {
		return response.Fail(c, err)
	}
